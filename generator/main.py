...
def _convert_to_gps(value, is_longitude):
    ...
    # Định dạng là (tử số, mẫu số)
    return {
        'value': (
            (degrees, 1),
            (minutes, 1),
            (int(seconds_float * 100), 100) # Lưu 2 chữ số thập phân cho giây
        ),
...
def create_exif_data(prefix, final_filename, exif_defaults, original_url=""):
    ...
    # Lấy thời gian hiện tại (UTC trên server) và trừ đi 2 giờ
